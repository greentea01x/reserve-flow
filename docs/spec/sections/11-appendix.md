...
| R-41 | Medium | ท้าทาย brief: สอง front-end apps สำหรับบริษัทเดียว = build/deploy/routing ซ้ำ | คง 2 SPA แต่ build เข้า Vercel project เดียว (`/` และ `/admin/`) + rewrite `/api` ไป Fly — admin bundle ไม่ไปอยู่ในเบราว์เซอร์พนักงาน และยังเป็น origin เดียว → 04, 07 |
...
| R-42 | Low | แผนสมมติ cloud (Vercel/Supabase) ทั้งที่ข้อจำกัด hosting / โดเมนอีเมลของบริษัทยังไม่รู้ | default: compose บน VM เดียว (SG/BKK) + SMTP relay ของบริษัท; ที่ตั้ง hosting + โดเมน/SPF/DKIM = ยืนยัน 11.H ข้อ 3 |
...
- **Vercel + Supabase** — เหตุผลเดิม ("ต้องการ worker ที่รันยาว") หมดไปเมื่อ Fly.io รับหน้าที่ API+worker แบบ always-on: **กลับคำโดย D-31 (2026-08-24)** — deploy จริงคือ Supabase + Fly.io + Vercel Hobby, compose เหลือ local dev เท่านั้น (ดู 11.B กลุ่ม 5)
...
:::details กลุ่ม 5 — มติลูกค้า 2026-08-24 และการตัดสินใจหลังปิดเล่ม (6 ข้อ)
...
| D-31 | สถาปัตยกรรม deploy | **Supabase free (`ap-southeast-1`) + Fly.io `sin` (API + worker, always-on, `min_machines_running=1`) + Vercel Hobby (สอง SPA + rewrite `/api` → Fly)** แทน VM เดียว + compose; compose เหลือ **local dev เท่านั้น** (postgres + mailpit); sweep/outbox/SMTP:587 ไม่เปลี่ยน; backup = nightly `pg_dump \| age` → R2 (Supabase free ไม่มี PITR); **ยอมรับความเสี่ยง ToS ของ Vercel Hobby (ห้ามใช้เชิงพาณิชย์) โดยชัดแจ้ง** — บันทึกเป็นความเสี่ยงที่รับไว้ ไม่ใช่ข้อที่มองข้าม | ต้นทุน ~$3–4/เดือน และชิ้นส่วนที่ต้อง patch เองน้อยกว่า VM; Fly ตอบข้อค้านเดิม "ต้องมี worker รันยาว" ที่เคยใช้ตัด Vercel/Supabase | ตัดสินใจแล้ว (2026-08-24) | แทนที่ D-23 และ ADR-008; ปิดข้อเปิดเรื่อง timezone (11.D ข้อ 6): server/DB = UTC + `timestamptz` แสดงผลที่ขอบด้วย `APP_TZ='Asia/Bangkok'`; SMTP relay + domain ยังต้องยืนยันกับ IT (11.H ข้อ 3) |
...
| origin เดียว | **Same-origin** | production ใช้ Vercel project เดียวเสิร์ฟ employee SPA ที่ `/`, admin SPA ที่ `/admin/` และ proxy rewrite `/api/*` ไป Fly; local ใช้ Vite proxy และ Fly image มีทั้งสอง SPA เป็น fallback/staging → browser ใช้ cookie origin เดียว ไม่มี CORS | 04 |
...
|---|---|---|
| ADR-001 | **Single origin** | สอง SPA + API ใช้ cookie session; production จึงใช้ Vercel project เดียว (`/`, `/admin/`, rewrite `/api/*` ไป Fly), local ใช้ Vite proxy, staging/fallback ให้ Fly image เสิร์ฟสอง SPA → cookie `__Host-sid`, SameSite=Lax + Origin check และไม่มี CORS. ถ้าแยก browser origin ภายหลังต้องออกแบบ CORS/CSRF ใหม่ (หัวข้อ 04, 09) |
...
| ADR-008 | **Superseded: VM + compose** | ข้อเสนอ VM เดิมถูกแทนที่โดย D-31 (2026-08-24) และไม่มี Caddy/prod compose ใน repo. Deploy ปัจจุบัน = Supabase + Fly.io + Vercel project เดียว; `infra/compose.yml` ใช้ local dev เท่านั้น. เก็บ ADR นี้เป็นประวัติ ไม่ใช่ runbook หรือตัวเลือกที่ระบบกำลังรัน |
:::
...
| 3 | ☐ | Domain + SMTP relay (D-23 → **แทนที่โดย D-31**: hosting ตัดสินแล้ว = Supabase SG + Fly `sin` + Vercel Hobby — ไม่ต้องถามที่ตั้ง VM อีก): ระบบเมล **Google Workspace หรือ M365**, บัญชี `noreply@<domain>` | Deploy ตาม D-31; ส่งเมลผ่าน relay ของบริษัท | ฝ่าย IT | W3 (SMTP ก่อน W4) | Workspace vs M365 = ความต่างระหว่างแก้ `.env` กับเขียน XOAUTH2 (11.D ข้อ 4); Postmark fallback ถ้าไม่มี relay |
