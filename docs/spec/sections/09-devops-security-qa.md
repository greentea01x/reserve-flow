...
หัวข้อนี้แยก **configuration ที่อยู่ใน repo** ออกจาก **external deployment state**: production target คือ Fly.io full stack + Supabase แต่ยังไม่ผ่าน unique credential onboarding, authenticated topology smoke และ restore drill จึงยังไม่ถือว่า production-ready

### 9.1 สภาพแวดล้อม (Environments)

Topology ที่ browser เห็นเหมือนกันทุก environment: **origin เดียว** `/` → `apps/web`, `/admin/` → `apps/admin` (Vite `base: '/admin/'`), `/api/` → Hono — เพราะ cookie `__Host-sid` ใบเดียวใช้ได้ทั้งสองแอป ไม่มี CORS ไม่มี cookie `Domain`; local ใช้ Vite proxy ส่วน staging/prod ให้ Fly เสิร์ฟทุกอย่างจาก image เดียว
...
| **prod target** | Fly app เดียว: employee/admin/API/jobs | `https://reserveflow-api.fly.dev` | Supabase target; backup restore ยังไม่พิสูจน์ | SMTP ยังไม่ยืนยัน | deployment ต้องผ่าน authenticated smoke |

ไม่ทำ: preview env ต่อ PR (ทีม 1–3 คนไม่คุ้ม), Supabase โปรเจกต์ที่สามสำหรับ drill (ใช้ local compose), serverless functions / Supabase Edge Functions / pg_cron (jobs ทุกตัวอยู่ใน process ของ API — หัวข้อ 05). Custom domain เป็น optional; SMTP relay ยังเป็นรายการ "ต้องยืนยันกับบริษัท" (หัวข้อ 11 · ภาคผนวก §11.H)

### 9.2 แพ็กเกจรันไทม์ (Dockerfile, fly.toml)

prod ไม่มี compose และไม่มี Caddy: ทั้งระบบคือ `apps/api/Dockerfile` ไฟล์เดียว (multi-stage, non-root, ถือ dist ของ web + admin และ API ไว้ในตัว) + `fly.toml` / `fly.staging.toml`; `infra/compose.yml` เหลือหน้าที่เดียวคือ **local dev** (postgres + mailpit) — ไฟล์จริงทั้งหมดอยู่ใน repo แล้ว: `fly.toml`, `fly.staging.toml`, `apps/api/Dockerfile`, `.github/workflows/{ci,deploy,backup}.yml`, `infra/supabase/bootstrap.sql`

:::details ดู fly.toml, Dockerfile และ bootstrap.sql
```toml
# fly.toml (ตัดมา) — app "reserveflow-api", region sin, full stack
[build]
  dockerfile = "apps/api/Dockerfile"
[env]
  PUBLIC_BASE_URL = "https://reserveflow-api.fly.dev"
[http_service]
  internal_port = 3000
  force_https = true
...
COPY --from=build /repo/apps/web/dist   ./apps/web/dist    # employee SPA ที่ /
COPY --from=build /repo/apps/admin/dist ./apps/admin/dist  # admin SPA ที่ /admin/
USER node
CMD ["node", "apps/api/dist/server.js"]
```

middleware ฝั่ง Hono ปิดช่องที่มากับ proxy: (1) `Cache-Control: no-store` บนทุก `/api/*` (2) request ที่ไม่ใช่ `/api` และ `Host` ≠ host ของ `PUBLIC_BASE_URL` → `308` ไป canonical origin (3) better-auth ใช้ `baseURL` จาก `PUBLIC_BASE_URL` เสมอ ไม่ derive จาก request เพราะ proxy อาจเขียน `Host` ทับ (`trustHost` ปิด)
...
| `PUBLIC_BASE_URL` | api | `http://localhost:5173` | `https://reserveflow-staging.fly.dev` | `https://reserveflow-api.fly.dev` | แหล่งเดียวของ canonical origin: cookie `Secure`, Origin allowlist, better-auth `baseURL`, 308 host-mismatch, ลิงก์อีเมล/.ics/QR; production value อยู่ใน `fly.toml` |
| `TRUST_PROXY` | api | false | true | true | อ่าน `X-Forwarded-For` จาก Fly proxy; IP เป็น **advisory** สำหรับ audit/log เท่านั้น ห้ามใช้ทำ IP allowlist (S-04) |
...
| Deploy secrets: `FLY_API_TOKEN`, `DATABASE_URL_MIGRATE` | GitHub repository หรือ Environment `production` (`deploy.yml`) | | | | workflow deploy full-stack image เดียว; frontend ไม่มี runtime secret |
...
**ตัวแปรที่หายไปจากแผน VM เดิม:** `UPLOADS_DIR` (รูปห้องเป็น `bytea` บนตาราง `rooms` — 3 ห้อง < 1 MB, อยู่ใน `pg_dump` อัตโนมัติ ไม่มี volume/เส้น backup ที่สอง), `POSTGRES_*`/`RF_OWNER_PASSWORD`/`RF_APP_PASSWORD` (กลายเป็น `infra/supabase/bootstrap.sql` รันครั้งเดียว; local ยังใช้ใน compose), `DOMAIN`/`ACME_EMAIL` (TLS เป็นของ Fly), `IMAGE_TAG` (Fly ถือ release history — rollback = `fly deploy --image <เดิม>`), `TZ` (UTC เสมอ), `SSH_HOST/USER/KEY` + `PROD_ENV_FILE`/`STAGING_ENV_FILE` (ไม่มี SSH และไม่มีไฟล์ `.env` ให้ ship)
...
Migration เป็น **forward-only** และรันก่อน Fly deploy ไม่รันตอน API boot; ทุกไฟล์ใช้ lock/statement timeout และต้อง backward-compatible กับ image ก่อนหน้าระหว่าง rolling deploy. **Rollback:** employee/admin/API/jobs เลือก Fly release/image ก่อนหน้าพร้อมกัน. ไม่มี pre-deploy dump ใน `deploy.yml`; schema error ให้ fix-forward เป็นหลัก ส่วน data corruption จึงหยุด API และ restore จาก backup workflow ล่าสุดตาม runbook/RPO โดยบันทึก incident และผลกระทบ
:::
...
| # | ด่านที่ต้องผ่าน |
|---|---|
| 1 | CI 4 jobs ที่มีจริงเขียวบน sha ที่ปล่อย; รัน dependency/image audit และ smoke ผ่าน Fly origin เป็นหลักฐาน manual (`/api`, deep link + asset ของ `/admin/`, `/`, และ `Cache-Control: no-store`) |
...
configuration ที่ commit อยู่เลือก **Fly.io `sin` + Supabase PostgreSQL `ap-southeast-1`**: browser เห็น `https://reserveflow-api.fly.dev` เป็น origin เดียวที่เสิร์ฟ employee/admin/API, process เดียวแบบ always-on ทำให้ sweep/outbox เดินต่อเนื่อง และ database เป็น managed PostgreSQL. ชื่อ plan, ราคา, quota และสถานะ provisioning เป็น external state ที่ repository ยืนยันไม่ได้และต้องตรวจใหม่ก่อน go-live

```mermaid
%% id: production-deployment
%% title: การติดตั้ง ReserveFlow บน Fly + Supabase
flowchart LR
  R --> G["GitHub Actions<br/>CI · migrate · deploy · backup"]
  U["ผู้ใช้ (Bangkok)"] --> A["Fly.io sin · reserveflow-api.fly.dev<br/>employee / · admin /admin/<br/>API /api/ · jobs (always-on)"]
...
สิ่งที่ควรอ่านจากภาพนี้: runtime path คือ Browser → Fly → Supabase; GitHub Actions ทำ migrate → deploy image เดียว → full-stack smoke. Backup ไม่ได้ไหลจาก Supabase ไป R2 โดยตรง: GitHub runner เป็นผู้ทำ `pg_dump`, เข้ารหัสด้วย `age` แล้ว upload ไป R2

**Vendor gate:** ก่อนใช้เชิงพาณิชย์ เจ้าของระบบต้องตรวจ plan/ToS/ราคาและ backup guarantees ล่าสุดของ Fly.io และ Supabase แล้วบันทึก plan ที่อนุมัติ; เอกสารนี้ไม่ตรึงราคาและไม่ถือ plan label เป็นข้อเท็จจริงถาวร

:::details เปรียบเทียบทางเลือก deploy และกับดักที่กันไว้แล้ว
| ทางเลือก | ~USD/เดือน | ภาระ ops | สรุป |
|---|---|---|---|
| **(a) Fly + Supabase** | ตรวจราคาปัจจุบันก่อนเปิดใช้ | `fly deploy`; backup ยังต้องเขียนเอง (GitHub Actions — งานนี้ไม่หายไปไหน) | **เลือก 2026-08-27** — employee/admin/API/jobs อยู่ใน image เดียว ไม่มี OS/DBA ให้ดูแล; ทุก guarantee ของสเปก (EXCLUDE A, advisory lock, sweep 60 s, outbox, SMTP 587) รันโดยไม่แก้โค้ดสักบรรทัด |
...
| (c) serverless functions เต็มตัว | ตรวจราคาปัจจุบัน | ต่ำสุดบนกระดาษ | **ไม่รับ** — ฆ่า in-process scheduler (sweep/outbox ต้องมี process ที่มีชีวิตระหว่าง request), cold start ชน p95, เปิด SMTP socket ต่อ invocation แลกกับ failure mode ใหม่หลายแบบ |

หมายเหตุจากการรีวิวข้อเสนอเดิม: sweep/outbox ต้องการ long-lived process จึงเลือก Fly แบบ always-on; topology origin เดียวตัด CORS และ cookie-domain complexity ออก. ตัด Neon เพราะ autosuspend 5 นาทีปิดไม่ได้ + งบ 100 CU-h/เดือนทำให้ keep-warm 24/7 ผิดกติกา — sweep ของเราจะปลุก DB ทั้งวัน; Supabase free ไม่หลับระหว่างวันจึงไม่ต้องแก้ design งานเลย
...
| X-Forwarded-For ผ่าน Fly proxy → caller spoof chain ได้ถ้าตั้ง trusted-proxy ผิด | IP = advisory; limiter คีย์ที่ identifier; ไม่มี IP allowlist (S-04) |
...
| นโยบาย/ราคา cloud เปลี่ยนได้จริง | ไม่มี SDK เฉพาะ platform (ปฏิเสธ Supabase Storage/Edge Functions ด้วยเหตุนี้); 1 Dockerfile + 1 Postgres URL = re-home ได้; ทบทวน ToS/ราคาเป็นรายไตรมาส (9.9) |
:::
...
| 5 | วัด p95 ของ `GET /api/v1/availability` จากเน็ตฝั่ง Bangkok ผ่าน `https://reserveflow-api.fly.dev`; เกิน budget ให้ตรวจ Fly region/size และ DB query ก่อน scale |
| 6 | manual deploy smoke ผ่าน Fly origin จริง: ตรวจ `Set-Cookie` ของ login ว่ามี `__Host-sid; Secure; Path=/; HttpOnly; SameSite=Lax` และ**ไม่มี `Domain=`**; `/api/*` ตอบ `Cache-Control: no-store` |
...
| S-04 | Rate limit + lockout | ตัวเลขตามหัวข้อ 06 C-13: sign-in 5/นาที ต่อ identifier (+IP เป็นเกณฑ์รอง — prod อยู่หลัง Fly proxy และ IP เป็น advisory), lockout 5 ครั้งล้มเหลว/15 นาทีต่อบัญชี → `423 ACCOUNT_LOCKED`; ข้อความ generic; dummy argon2 verify เมื่อไม่พบผู้ใช้ (กัน enumeration/timing); check-in 10/นาที/user; `POST /bookings` 30/นาที/user; resend-invite/reset-password ใช้ budget ร่วม 3/ชม. ต่อ user. ตัว limiter เป็น helper in-process ใน `lib/rate-limit.ts`; API จึงต้องคง single instance หรือย้าย state ลง PostgreSQL ก่อน scale-out | credential stuffing ในออฟฟิศ 80 คน | TC-RATE-024 |
...
| S-16 | Headers | Hono ใช้ `secureHeaders()` ก่อน static/API routing จึงครอบ `/`, `/admin/` และ `/api/`; `/api/*` เพิ่ม `Cache-Control: no-store`. ตรวจ header จริงบน `/`, `/admin/`, `/api/healthz` ก่อน go-live | runtime เดียวทำให้ policy header สอดคล้องกันทุก route | TC-SEC-021 |
...
| S-18 | Env validation + error hygiene | zod env ตอน boot (ชื่อไม่ใช่ค่า); ไม่มี stack trace ถึง client, body `{code,message,details,request_id}`; timing-safe compare ทุก token; container non-root (`USER node`); browser ใช้ `PUBLIC_BASE_URL` เป็น canonical Fly origin และ request non-API ที่ host อื่นถูก `308` กลับ (9.2) | fail fast ดีกว่า fail ตอน 09:00 | TC-OPS-026, TC-SEC-021 |
:::
...
| Manual browser | in-app browser/เบราว์เซอร์จริงผ่าน Fly origin | ก่อน go-live และเมื่อ flow/UI เปลี่ยน | employee/admin journeys, same-origin routing, deep links, cookie/header, QR check-in และ failed-email retry |
...
- **Request id:** API สร้าง/echo `X-Request-Id` ใน response, business error body และ audit row ที่ mutation ส่งต่อ; logger ยังไม่เติม platform request id แยกต่างหาก
...
- **สิ่งที่คนดู:** `fly logs`, Fly health, Supabase usage/status, backup heartbeat และ admin email queue. external uptime/error tracker เป็น Phase 2; เมื่อเปิดใช้ต้อง ping ผ่าน Fly origin จริงและเพิ่ม runbook/owner ก่อนเรียกว่า alert path
